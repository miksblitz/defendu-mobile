...
export function compareRepWithFeedbackSlip(
  ...
  const formCheck = isSlipFormAcceptable(userFrames, referenceFrames);
...
export function compareRepWithFeedbackAnySlip(
  ...
  for (const ref of referenceSequences) {
    ...
    const formCheck = isSlipFormAcceptable(userFrames, ref);
  ...
  const formCheck = isSlipFormAcceptable(userFrames, bestRef);
