...
import { isLeadElbowStrikeFormAcceptable } from './leadElbowStrikeFeedback';

export function compareRepWithFeedbackLeadElbowStrike(
  ...
  const form = isLeadElbowStrikeFormAcceptable(userFrames);
  return { match: form.acceptable, distance, feedback: form.feedback };
}

export function compareRepWithFeedbackAnyLeadElbowStrike(
  userFrames: PoseFrame[],
