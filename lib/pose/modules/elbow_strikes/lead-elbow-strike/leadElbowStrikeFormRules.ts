...
/** Opposite (left) shoulder index, used only to estimate body size for far-distance handling. */
const MP_OPPOSITE_SHOULDER = 11;
const MN17_OPPOSITE_SHOULDER = 5;
...
export function getLeadArmSnapshot(frame: PoseFrame): LeadArmSnapshot | null {
  if (frame.length > 17) {
    return metricsFromIndices(frame, MP.rs, MP.re, MP.rw, MP_OPPOSITE_SHOULDER);
  }
  return metricsFromIndices(frame, MN17.rs, MN17.re, MN17.rw, MN17_OPPOSITE_SHOULDER);
}

/**
 * Snapshot of the OPPOSITE arm (user's RIGHT = MediaPipe LEFT 11/13/15) using
 * the EXACT same metric calculations as the lead arm. Used so that:
 *   - Wrong-arm detection (opposite arm performing the strike) uses the same
 *     relaxed angle/lateral/far-distance rules as the perfect rep.
 *   - Guard-down detection can read shoulder–wrist distance and wrist height.
 *
 * The "opposite shoulder" passed for body-scale is MP.rs / MN17.rs (the lead
 * arm's shoulder), so bodyScale is identical regardless of which arm is being
 * snapshotted.
 */
export function getOppositeArmSnapshot(frame: PoseFrame): LeadArmSnapshot | null {
  if (frame.length > 17) {
    return metricsFromIndices(frame, MP_OPPOSITE_SHOULDER, 13, 15, MP.rs);
  }
  return metricsFromIndices(frame, MN17_OPPOSITE_SHOULDER, 7, 9, MN17.rs);
}
 ...
 */
export function isLeadElbowStrikeAlignedAndFlaredFarDistance(s: LeadArmSnapshot): boolean {
...
export function isLeadElbowFinalPose(s: LeadArmSnapshot | null, _leadContracting: boolean): boolean {
  if (!s) return false;
  return (
    isLeadElbowStrikeAlignedAndFlared(s) ||
    isLeadElbowStrikeAlignedAndFlaredFarDistance(s)
  );
}
