...
import { isFacingRightSideStrict } from '../../punching/facingDirection';
...
export function createLeadElbowStrikeRepDetector(): (frame: PoseFrame, now: number) => RepDetectorResult {
  ...
  return function tick(frame: PoseFrame, now: number): RepDetectorResult {
    if (isFacingRightSideStrict(frame) && now >= rightFacingBadUntil) {
      ...
      return {
        ...
        forcedBadRep: true,
        feedback: [{
          id: 'lead-elbow-strike-facing-right-bad-rep',
          message: 'FACE FRONT!',
          severity: 'error',
          phase: 'impact',
        }],
