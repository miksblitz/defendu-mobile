/**
 * Lead uppercut elbow strike comparison (STRICT).
 */
...
import { isImpactFormAcceptableLeadUppercutElbowStrikeStrict } from './leadUppercutElbowStrikeFeedbackStrict';

export function compareRepWithFeedbackLeadUppercutElbowStrikeStrict(
  ...
  const impactCheck = isImpactFormAcceptableLeadUppercutElbowStrikeStrict(userFrames, referenceFrames, refBounds);
  return { match: impactCheck.acceptable, distance, feedback: impactCheck.feedback };
}

export function compareRepWithFeedbackAnyLeadUppercutElbowStrikeStrict(
  userFrames: PoseFrame[],
  ...
  const impactCheck = isImpactFormAcceptableLeadUppercutElbowStrikeStrict(userFrames, bestRef, refBounds);
