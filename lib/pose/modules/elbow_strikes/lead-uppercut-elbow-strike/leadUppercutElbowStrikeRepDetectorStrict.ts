...
function isStrikeAttemptAndGuardOk(frame: PoseFrame): boolean {
  const strike = bestRightStrikeMetrics(frame);
  const guard = bestLeftGuardMetrics(frame);
  if (!strike || !guard) return false;
  ...
  return (
    ...
    guard.wristAboveElbow >= GUARD_UP_MIN_WRIST_ABOVE_ELBOW
  );
}
...
export function createLeadUppercutElbowStrikeRepDetectorStrict(): (frame: PoseFrame, now: number) => RepDetectorResult {
  ...
  return function tick(frame: PoseFrame, now: number): RepDetectorResult {
    ...
    if (state === 'idle') {
      if (!attemptOk) return { done: false };
      state = 'holding';
      segment = [frame];
      return { done: false };
    }
