/**
 * High lead knee strike — distance for diagnostics; "match" follows form (hip-line raise)
 * so reference timing doesn't block a good rep.
 */

import type { PoseFrame, PoseFeedbackItem, PoseSequence, PoseFocus } from '../../../types';
import { compareRepsWithFocus } from '../../../comparator';
import { getHighLeadKneeStrikeFormFeedback } from './highLeadKneeStrikeFeedback';

export function compareRepWithFeedbackHighLeadKneeStrike(
  userFrames: PoseFrame[],
  referenceFrames: PoseSequence,
  _threshold: number = 0.2,
  focus?: PoseFocus
): { match: boolean; distance: number; feedback: PoseFeedbackItem[] } {
  const form = getHighLeadKneeStrikeFormFeedback(userFrames);
...
export function compareRepWithFeedbackAnyHighLeadKneeStrike(
  userFrames: PoseFrame[],
