/**
 * High lead knee strike — rep detector.
 *
 * Right leg only: if the right knee is visibly above the mid-hip horizontal line (MediaPipe
 ...
 *
 * Uses hysteresis so the line crossing doesn't chatter.
 */
...
export function createHighLeadKneeStrikeRepDetector(): (frame: PoseFrame, now: number) => RepDetectorResult {
  ...
  return function tick(frame: PoseFrame, now: number): RepDetectorResult {
    ...
    if (phase === 'cooldown') {
      ...
      // Require knees back below the line so the next rep is a distinct motion.
