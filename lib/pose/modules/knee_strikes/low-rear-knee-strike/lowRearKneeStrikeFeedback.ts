/**
 * Low rear knee strike — form: left knee on/below hip line + bent-knee window heuristics.
 */
...
export function getLowRearKneeStrikeFormFeedback(
  ...
  for (const f of userFrames) {
    if (inLowRearStrikePose(f, idx)) sawStrikePose = true;
    if (leftKneeOnOrBelowMidHip(f, idx)) sawUnderHip = true;
    const a = leftKneeAngleDeg(f, idx);
  ...
  if (!sawUnderHip) {
    feedback.push({
      id: 'knee-under-hip',
      message: 'Keep your left knee on or below your hip line (don’t lift it above hip height)',
      severity: 'hint',
    });
  }
  ...
  if (!sawStrikePose) {
    feedback.push({
      id: 'low-rear-pose',
      message: 'Bend the left knee for a low chamber while staying under the hip line',
      severity: 'hint',
    });
  }
