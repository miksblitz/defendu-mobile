 ...
 * Jab-specific form rules and orthodox (left punch, right guard) checks.
...
export function leadArm(frame: PoseFrame): 0 | 1 {
...
/** Body-agnostic: only arm straightness and extension. Facing camera or not doesn't matter. */
const RULES = {
  /** Straight arm: ~parallel to shoulder line; 140° = almost straight */
  minElbowAngleAtImpact: 140,
  /** Extension: only fail if arm is barely extended (very low bar) */
...
export function compareJabMetrics(
  ...
  if (reference) {
    // Arm only: straight elbow and extension. Body facing / guard / stance ignored.
    if (user.punchingElbowAngle != null) {
      const minAngle = Math.max(RULES.minElbowAngleAtImpact, (reference.punchingElbowAngle ?? 180) - 25);
      if (user.punchingElbowAngle < minAngle) {
        feedback.push({
          id: 'elbow-not-straight',
          message: 'Straighten the punching arm more at extension',
          phase,
          severity: user.punchingElbowAngle < RULES.minElbowAngleAtImpact ? 'error' : 'warning',
        });
      }
    }
    ...
    // Pure rule-based (no reference): arm straight + extended only
...
export function getJabFeedback(
  ...
  const userImpactFrame = userFrames[maxIdx] ?? userFrames[userFrames.length - 1]!;
...
/** Only these count as errors. Body position/guard ignored—only arm straightness and extension. */
const UPPER_BODY_ERROR_IDS = ['front-hand-not-extended', 'elbow-not-straight'];
...
// --- Orthodox jab only: user's LEFT hand extends, user's RIGHT hand stays in guard. ---
// App UI shows "Your left" = extensionValues.right (MediaPipe right). So: punch = MediaPipe right, guard = MediaPipe left.

const ORTHODOX_GUARD_MAX_EXTENSION = 0.24;  // guard arm (user's right = MediaPipe left) wrist–shoulder
const ORTHODOX_WRIST_UP_TOL = 0.14;         // guard: wrist y <= elbow y + this

/** Guard = user's right hand = MediaPipe LEFT (indices 11,13,15). */
function leftHandInGuardFromFrame(frame: PoseFrame): { inGuard: boolean; extended: boolean; wristDown: boolean } {
  const d = armExtensionDistances(frame);
  if (!d) return { inGuard: false, extended: false, wristDown: false };
  const ls = frame[11]; // LEFT_SHOULDER (user's right = guard)
  const le = frame[13]; // LEFT_ELBOW
  const lw = frame[15]; // LEFT_WRIST
  if (!ls || !le || !lw || frame.length <= 15) return { inGuard: false, extended: false, wristDown: false };
  const extended = d.left > ORTHODOX_GUARD_MAX_EXTENSION;
  const wristUp = lw.y <= le.y + ORTHODOX_WRIST_UP_TOL;
  const inGuard = d.left <= ORTHODOX_GUARD_MAX_EXTENSION && wristUp;
  return { inGuard, extended, wristDown: !wristUp };
}

/**
 * Orthodox jab: punching arm must be user's LEFT = MediaPipe right = leadArm 1. Guard = user's right = MediaPipe left.
 */
export function getJabFeedbackOrthodox(
  userFrames: PoseFrame[],
  referenceFrames: PoseFrame[] | null,
  referencePhaseBounds?: { phase: JabPhase; start: number; end: number }[]
): PoseFeedbackItem[] {
  ...
  const guard = leftHandInGuardFromFrame(frame); // user's right hand = MediaPipe left
  const ext = armExtensionDistances(frame);

  // Require user's LEFT to punch = MediaPipe right extending = leadArm 1. If leadArm 0, they're jabbing with right (wrong).
  if (metrics.leadArm === 0) {
    base.push({
      id: 'wrong-arm',
      message: 'Jab with your left hand; keep right in guard (orthodox stance)',
      phase: 'impact',
      severity: 'error',
    });
  }

  if (guard.extended && ext && ext.left > ORTHODOX_GUARD_MAX_EXTENSION) {
    base.push({
      id: 'rear-hand-not-in-guard',
      message: 'Keep right hand in guard (contracted, wrist up)',
      phase: 'impact',
      severity: 'error',
    });
  }
  if (guard.wristDown && !guard.extended) {
    base.push({
      ...
      message: 'Keep right hand wrist up in guard',
      phase: 'impact',
      severity: 'error',
    });
  }
...
/** Error ids for orthodox jab (stricter: no wrong arm, rear hand must be in guard). */
const ORTHODOX_ERROR_IDS = [
  ...UPPER_BODY_ERROR_IDS,
  'wrong-arm',
  'rear-hand-not-in-guard',
  'rear-hand-wrist-down',
];

/** Orthodox jab: no errors allowed on arm choice or guard; pass only when form is correct. */
const ORTHODOX_MAX_ERRORS_TO_PASS = 0;

/**
 * Judge rep for orthodox jab: left extends, right in guard (contracted, wrist up). Stricter.
 */
export function isImpactFormAcceptableOrthodox(
  userFrames: PoseFrame[],
  referenceFrames: PoseFrame[] | null,
  referencePhaseBounds?: { phase: JabPhase; start: number; end: number }[]
): { acceptable: boolean; feedback: PoseFeedbackItem[] } {
  const feedback = getJabFeedbackOrthodox(userFrames, referenceFrames, referencePhaseBounds);
  const errorCount = feedback.filter(
    (f) => f.severity === 'error' && ORTHODOX_ERROR_IDS.includes(f.id)
  ).length;
  return { acceptable: errorCount <= ORTHODOX_MAX_ERRORS_TO_PASS, feedback };
}
